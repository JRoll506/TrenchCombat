...
public class Shotgun extends EngineClass {
	...
	public Shotgun() {
		super(new ItemStack[] { new ItemStack(Material.IRON_SWORD), new ItemStack(Material.BONE), Game.getPlugin().getGrenade() }, "shotgun");
		String[] description = { 
				"&2=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=", 
				"&f&lShotgun Class",
	...
	public void onInteract(PlayerInteractEvent event) {
		...
		if (event.getMaterial().name() == "BONE") {
			if (Game.game.cooldownShotgun.toArray().length == 0) {
				fireArrow(player);
			} else {
				if (!(Game.game.cooldownShotgun.contains(player))) {
					fireArrow(player);
				}
			}
	...
	public void fireArrow(final Player player) {
		Game.game.cooldownShotgun.add(player);
		...
		scheduler.scheduleSyncDelayedTask(Game.getPlugin(), new Runnable() {
			public void run() {
				Game.game.cooldownShotgun.remove(player);
