...
public class EngineGame {
	...
	public void tpPlayers() {
		for (Player player : Bukkit.getOnlinePlayers()) {
			if (getColorTeam(player) == ColorTeam.RED) {
				player.teleport(Game.trenchConfig.getRed());
			} else {
				player.teleport(Game.trenchConfig.getBlue());
			}
		}
	}
	...
	public void giveItems() {
		for (Player player : Game.getPlugin().getServer().getOnlinePlayers()) {
			...
			if (Game.getClassManager().getClass(player) == null) {
				player.getInventory().addItem(Game.getClassManager().getEngineClass("Gunner").getItems());
				player.addPotionEffect(Game.getClassManager().getEngineClass("Gunner").getEffect());
				continue;
			}
			player.getInventory().addItem(Game.getClassManager().getEngineClass(Game.getClassManager().getClass(player)).getItems());
			player.addPotionEffect(Game.getClassManager().getEngineClass(Game.getClassManager().getClass(player)).getEffect());
		}
	}
	...
	public void setScoreBoard() {
		...
		String second = Game.clock.getGameClock() % 60 + "";
		if (Game.clock.getGameClock() % 60 <= 9)
			second = "0" + second;
		String time = (Game.clock.getGameClock() - (Game.clock.getGameClock() % 60)) / 60 + ":" + second;
	...
	public void giveItems(final Player player) {
		...
		player.getInventory().addItem(Game.getClassManager().getEngineClass(Game.getClassManager().getClass(player)).getItems());
		Game.getPlugin().getServer().getScheduler().scheduleSyncDelayedTask(Game.getPlugin(), new Runnable() {
			public void run() {
				player.addPotionEffect(Game.getClassManager().getEngineClass(Game.getClassManager().getClass(player)).getEffect());
			}
		}, 20);
